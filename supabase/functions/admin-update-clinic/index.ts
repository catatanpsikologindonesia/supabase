...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const ipLimit = await checkRateLimit(service, `ip:${ip}`, 'admin-update-clinic', 10, 20);
    if (ipLimit.limited) {
    ...
    const actorLimit = await checkRateLimit(service, `actor:${user.id}`, 'admin-update-clinic', 10, 15);
    ...
    const { data: updateResult, error: updateError } = await service.from('clinics').update({
